...
import SwiftUI
#if canImport(AppKit)
struct NavigationBarPassThroughInstaller: NSViewRepresentable {
    func makeNSView(context: Context) -> NSView {
        let view = NSView()
        Task { @MainActor in
            guard let window = view.window else { return }
            ClickThroughRegistry.shared.ensureSwizzleInstalled {
                installNavigationBarPassThroughSwizzle()
            }
...
struct NavigationBarPassThroughInstaller: UIViewRepresentable {
